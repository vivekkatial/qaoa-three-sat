...
def create_nelder_mead_opts(budget):
    ...
    optimisation_opts = (
        "{'classical_opt_alg':'nelder-mead', 'xtol': 0.001, 'disp' : True, 'adaptive' : True, 'budget':%s}"
        % (budget)
    )
...
def main():
    ...
    for inst in product(l_classical_opt_alg, l_budget, l_n_rounds):
        ...
        if int(inst[1]) >= 2000*int(inst[2]):

            instance = params_template
            instance["classical_optimisation"]["classical_opt_alg"] = inst[0]
            instance["classical_optimisation"][
                "optimisation_opts"
            ] = create_optimisation_dict(inst[0], inst[1])
